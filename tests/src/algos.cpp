...
namespace
{
	...
	void test_det(std::string_view test_name)
	{
		test(test_name.data()) = [test_name]<typename Mat>(std::type_identity<Mat>) {
			...
			const auto out        = det(mat, std::type_identity<To>{});
	...
	void test_lu(std::string_view test_name)
	{
		test(test_name.data()) = [test_name]<typename Mat, typename Mat2>(
									 std::tuple<std::type_identity<Mat>, std::type_identity<Mat2>>) {
			const auto [mat, mat2, mat3] = parse_test(test_name, parse_mat<Mat>, parse_mat<Mat2>, parse_mat<Mat2>);
			const auto [out, out2]       = lu(mat, std::type_identity<Mat2>{});
	...
	void test_block(std::string_view test_name)
	{
		test(test_name.data()) = [test_name]<typename Mat, typename Mat2>(
			...
			const auto out =
				block(mat, idxs.row_start, idxs.col_start, idxs.row_end, idxs.col_end, std::type_identity<Mat2>{});
...
{
	feature("Determinant") = []() {
		test_det<all_mats<float, 0, 0>, float>("algos/det/0x0.txt");
		test_det<all_mats<float, 1, 1>, float>("algos/det/1x1.txt");
		test_det<all_mats<double, 2, 2>, double>("algos/det/2x2.txt");
		test_det<all_mats<float, 3, 3>, float>("algos/det/3x3.txt");
	...
	feature("Inverse") = []() {
		test_fn<join_mats<all_mats<float, 0, 0>, all_mats<float, 0, 0>>>("algos/inv/0x0.txt", inv);
		test_fn<join_mats<all_mats<float, 1, 1>, all_mats<float, 1, 1>>>("algos/inv/1x1.txt", inv);
		test_fn<join_mats<all_mats<float, 2, 2>, all_mats<float, 2, 2>>>("algos/inv/2x2.txt", inv);
		test_fn<join_mats<all_mats<float, 3, 3>, all_mats<float, 3, 3>>>("algos/inv/3x3_int.txt", inv);
		test_fn<join_mats<all_mats<double, 3, 3>, all_mats<double, 3, 3>>>("algos/inv/3x3.txt", inv);
		test_fn<join_mats<all_mats<float, 10, 10>, all_mats<float, 10, 10>>>("algos/inv/10x10.txt", inv);
	};
	...
	feature("LU Decomposition") = []() {
		test_lu<join_mats<all_mats<double, 2, 2>, all_mats<double, 2, 2>>>("algos/lu/2x2.txt");
		test_lu<join_mats<all_mats<double, 3, 3>, all_mats<double, 3, 3>>>("algos/lu/3x3.txt");
	};
