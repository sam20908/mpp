...
namespace matrixpp
{
	namespace detail
	{
		inline auto add_op = [](auto&& left, auto&& right, std::size_t row_idx, std::size_t col_idx) {
			return left(row_idx, col_idx) + right(row_idx, col_idx);
		};

		using add_op_type = decltype(add_op);
	} // namespace detail
		...
		const detail::expr_base<RightBase, Value, RowsExtent, ColumnsExtent>& right) // @TODO: ISSUE #20
	{
		...
		return detail::expr_binary_op<RowsExtent, ColumnsExtent, left_type, right_type, detail::add_op_type>{ left,
			right,
			detail::add_op,
			left.rows(),
			left.columns() };
		...
		const detail::expr_base<RightBase, Value, RightRowsExtent, RightColumnsExtent>& right) // @TODO: ISSUE #20
	{
		...
		return detail::
			expr_binary_op<std::dynamic_extent, std::dynamic_extent, left_type, right_type, detail::add_op_type>{ left,
				right,
				detail::add_op,
				left.rows(),
				left.columns() };
