...
namespace mpp
{
	template<detail::arithmetic Val, std::size_t Cols, typename Alloc>
	class mat<Val, dyn, Cols, Alloc> :
		public detail::mat_dyn_base<mat<Val, dyn, Cols, Alloc>,
			typename cfg<override>::dyn_rows_buf<Val, 1, Cols, Alloc>,
			Val,
			dyn,
			Cols,
			Alloc>
	{
		using base = detail::mat_dyn_base<mat<Val, dyn, Cols, Alloc>,
			typename cfg<override>::dyn_rows_buf<Val, 1, Cols, Alloc>,
			Val,
			dyn,
			Cols,
	...
	public:
		...
		mat() noexcept(std::is_default_constructible_v<Alloc>) : base(0, Cols, Alloc{}) {} // @TODO: ISSUE #20

		explicit mat(const Alloc& alloc) noexcept : base(0, Cols, alloc) {} // @TODO: ISSUE #20
		...
		requires(!std::same_as<std::remove_cvref_t<Mat>, mat<Val, dyn, Cols, Alloc>>) explicit mat(Mat&& obj,
			const Alloc& alloc = Alloc{}) :
			base(0, Cols, alloc) // @TODO: ISSUE #20
		{
		...
		mat(std::size_t rows, const Alloc& alloc = Alloc{}) :
		...
		explicit mat(Rng&& rng, const Alloc& alloc = Alloc{}) : base(0, Cols, alloc) // @TODO: ISSUE #20
		{
		...
		explicit mat(const detail::expr_base<Expr, Val, ExprRowsExtent, ExprCols>& expr, const Alloc& alloc = Alloc{}) :
			base(0, Cols, alloc) // @TODO: ISSUE #20
		{
			base::init_from_expr_dyn(expr.rows(), Cols, expr);
		}

		mat(std::size_t rows, const Val& val, const Alloc& alloc = Alloc{}) :
			base(rows, Cols, rows * Cols, val, alloc) // @TODO: ISSUE #20
		{
		}
		...
		mat(std::size_t rows, Fn&& fn, const Alloc& alloc = Alloc{}) : base(rows, Cols, alloc) // @TODO: ISSUE #20
		{
		...
		template<detail::matrix_with_value_convertible_to<Val> Mat>
		requires(!std::same_as<std::remove_cvref_t<Mat>, mat<Val, dyn, Cols, Alloc>>) void assign(Mat&& obj)
		{
			const auto rows = std::forward<Mat>(obj).rows();
			const auto cols = std::forward<Mat>(obj).cols();
